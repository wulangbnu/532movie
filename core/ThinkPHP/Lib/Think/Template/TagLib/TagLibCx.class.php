...
{//类定义开始	
	...
	public function _gxcmsfor($attr,$content,$gxtag = 'gxcms') {
		...
		$s = '$';
		foreach($tag as $keyy=>$value){
			$_tag .= $s."tag['$keyy'] = '".$value."';";
		}
		if('menu' == $table){
			$_function = 'get_tag_gxmenu';
		}else{
			$_function = 'get_tag_'.$gxtag;
		}
        $parseStr  =  '<?php '.$_tag.' $__LIST__ = '.$_function.'($tag); if(is_array($__LIST__)): $'.$key.' = 0;';
        $parseStr .= 'if( count($__LIST__)==0 ) : echo "'.$empty.'" ;';
        $parseStr .= 'else: ';
        $parseStr .= 'foreach($__LIST__ as $key=>$'.$table.'): ';
        $parseStr .= '++$'.$key.';';
